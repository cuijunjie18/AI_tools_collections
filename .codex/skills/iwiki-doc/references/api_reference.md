...
### getDocument
获取 iWiki 文档的完整 Markdown 格式内容。
**参数：**
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| docid | string | ✅ | 文档 ID |

**返回：** 文档的完整 Markdown 内容（包括标题、正文、图片、表格等）

### metadata
获取 iWiki 文档元数据，包含基础属性信息。
**参数：**
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| docid | string | ✅ | 文档 ID |
...
### getSpacePageTree
获取空间的目录树信息，返回指定父级文档下的所有子文档列表。
**参数：**
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| parentid | number | ✅ | 父级文档 ID，作为获取子文档的起始节点 |
...
### saveDocumentParts
局部更新文档，支持在文档开头插入内容或在文档结尾追加内容。支持 DOC 和 MD 类型文档。DOC 类型追加 HTML 格式内容，MD 类型追加 Markdown 格式内容。
**参数：**
| 参数 | 类型 | 必填 | 描述 |
|------|------|------|------|
| id | number | ✅ | 文档 ID |
...
```json
{
  ...
  "comment": [
    {
      "id": 10000921844,
      "content": "批注内容（ADF JSON 字符串）",
      "creator": "zhangsan",
      "display_name": "zhangsan(张三)",
      "createtime": "2026-04-24 15:37:09",
      "parent_id": 0,
      "reply_to": "lisi(李四)",
      "mark_content": "划词原文",
      "is_deleted": false,
      ...
      "next_level_comments": [
        {
          "id": 10000921845,
          "content": "回复内容（ADF JSON 字符串）",
          "creator": "zhangsan",
          "display_name": "zhangsan(张三)",
          "createtime": "2026-04-24 15:37:11",
          "parent_id": 10000921844,
          "reply_to": "zhangsan(张三)",
          "is_deleted": false,
          "next_level_total": 0
        }
      ]
...
- `total`：顶级划词批注的总数
- `id`：批注 ID
- `content`：批注内容（ADF / teditor JSON 字符串，正文文本在 `content[].content[].text` 中）
- `creator` / `display_name`：创建者账号 / 显示名
- `createtime`：创建时间
- `parent_id`：父批注 ID，顶级批注为 0
- `reply_to`：回复对象
- `mark_content`：划词原文（仅顶级批注有）
- `is_deleted`：是否已删除
- `next_level_total`：子回复数量
- `next_level_comments`：嵌套的回复列表，结构同上，可递归
